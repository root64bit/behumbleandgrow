# Schema & Data Mapping — Candidate Interview Details

## Database Tables & Relational Mapping

| Detail Concern | Actual Source Table | Ownership Relationship | Candidate-Visible Fields | RLS Policy Rule |
|---|---|---|---|---|
| Interview Record | `interviews` | `candidate_id = auth.uid()` via `applications.candidate_id` | `id`, `application_id`, `scheduled_at`, `duration_minutes`, `status`, `format`, `location` | Candidate selects own interviews |
...
| Meeting Access | Server-Side RPC / Verification | Auth session + server time check | Expiring one-time access token / redirect URL | Access window validation |

## Field Projection Rules
- **Allowed**: `id`, `application_id`, `job_title`, `employer_display_name`, `scheduled_at`, `duration_minutes`, `format`, `status`, `location`, `instructions`, `preparation_items`, `required_documents`.
