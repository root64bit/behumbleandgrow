...
import { PartnerActionItem } from '../../types/recruiter';
import { AlertCircle, Clock, ChevronRight, CheckCircle2 } from 'lucide-react';
import { Link } from 'react-router-dom';

interface PartnerActionCentreProps {
  items: PartnerActionItem[];
}

export default function PartnerActionCentre({ items }: PartnerActionCentreProps) {
    ...
    <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/80 shadow-xs space-y-4 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base font-extrabold text-slate-900">Priority Action Queue</h2>
            <p className="text-xs text-slate-500">Urgent lead acceptances, candidate clarifications, and interview feedback items.</p>
          </div>
        </div>
          ...
          >
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full border ${getPriorityBadge(item.priority)}`}>
                  {item.priority}
                </span>
              </div>
