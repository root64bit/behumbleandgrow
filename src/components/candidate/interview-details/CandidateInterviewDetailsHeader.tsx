...
export const CandidateInterviewDetailsHeader: React.FC<Props> = ({ interviewId }) => {
  ...
  return (
    <div className="flex items-center justify-between pb-3 border-b border-slate-100 text-left">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-emerald-50 text-[#006D44] border border-emerald-200 rounded-full text-[10px] font-extrabold uppercase tracking-wider">
            <Video className="w-3 h-3 text-[#006D44]" />
            <span>Interview Details</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-[#00122B] mt-0.5 tracking-tight">
            Interview #{interviewId ? interviewId.slice(0, 8).toUpperCase() : '---'}
          </h1>
        </div>
      </div>
