...
interface CandidateSupportConflictStateProps {
  onRefresh: () => void;
}

export function CandidateSupportConflictState({ onRefresh }: CandidateSupportConflictStateProps) {
  return (
    <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl flex items-center justify-between gap-3 text-xs text-amber-900 dark:text-amber-200">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold">
          This support request was modified in another session. Refreshing details.
        </span>
      </div>
      <button
        onClick={onRefresh}
